...
These classes accept a custom three-way comparison operator when constructed. For example, this *max-heap* compares objects by their `.key`:
...
Returns `True` if and only if the two heaps are of the same type, use the same custom comparator configuration, and contain equivalent [values](#method_values). Note that a concrete heap is of a different type than a role, so:
...
Defined as:
    ...
    method new(*@values, :&comparator --> BinaryHeap:D)

Constructs a new heap storing the provided values. `BinaryHeap::MaxHeap` and `BinaryHeap::MinHeap` accept an optional custom three-way `&comparator`.
...
[Simplified by Anton Antonov in August 2026](https://github.com/antononcube/Raku-BinaryHeap).
